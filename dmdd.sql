...
Insert into REQSTATUSTYPE (REQSTATUSNO,STATUSTYPE) values (601,'available');
Insert into REQSTATUSTYPE (REQSTATUSNO,STATUSTYPE) values (602,'unavailable');
Insert into REQSTATUSTYPE (REQSTATUSNO,STATUSTYPE) values (603,'unavailable');
Insert into REQSTATUSTYPE (REQSTATUSNO,STATUSTYPE) values (604,'available');
Insert into REQSTATUSTYPE (REQSTATUSNO,STATUSTYPE) values (605,'available');
...
---create view for blood request location
create or replace view BLOOD_REQUEST_LOCATION AS
select l.location_name, h.hospital_name, br.qtyreq, br.reqpriority, br.requestdate as Date_req, rst.statustype from 
 location l, hospital h, bloodrequest br, reqstatustype rst
 where 
 l.location_id = h.location_id and h.hospital_id = br.hospital_id and br.reqstatusno = rst.reqstatusno;
 
select * from blood_request_location;

--create view for the blood requests which are open

create or replace view openrequestss as 
...
select * from openrequestss;


--create view for requests which are closed

create or replace view closedrequests as 
 select h.hospital_name,p.patient_name,bl.bloodgroup_name,br.REQUESTDATE,rs.statustype from bloodrequest br inner join hospital h on h.hospital_id=br.hospital_id 
 inner join patient p on p.patient_id=br.patient_id 
 inner join reqstatustype rs 
 on rs.reqstatusno = br.reqstatusno 
